...
module.exports = function (setup) {
  ...
  module.findOrCreateUser = function (
    users,
    refreshToken,
    characterDetails,
    cb
    ...
    if (refreshToken) {
      db.updateOne(
        ...
        function (err, result) {
          console.info(
            "users.findOrCreateUser: Updating refreshToken for " +
              characterDetails.CharacterName
          );
        }
      );
    } //Check if the user exists
    module.findAndReturnUser(characterDetails.CharacterID, function (
      userProfile
    ) {
      ...
      if (userProfile) {
        ...
        module.generateNewUser(refreshToken, characterDetails, function (
          userProfile,
          err
        ) {
          cb(userProfile, err);
  ...
  module.findAndReturnUser = function (checkID, cb) {
    db.find({ characterID: checkID }).toArray(function (err, docs) {
      if (err)
        log.error("findAndReturnUser: Error for db.find.toArray", {
          err,
          checkID,
        });
      if (docs.length === 0) {
  ...
  module.generateNewUser = function (refreshToken, characterDetails, cb) {
    module.getPilotAffiliation(characterDetails.CharacterID, function (
      alliance,
      corporation
    ) {
      var newUserTemplate = {
      ...
      db.insert(newUserTemplate, function (err, result) {
        if (err)
          log.error("generateNewUser: Error for db.insert", {
            err,
            name: characterDetails.CharacterName,
          });
        cb(newUserTemplate);
  ...
  module.updateUserPermission = function (user, permission, adminUser, cb) {
    ...
    if (user.characterID !== adminUser.characterID) {
      db.updateOne(
        ...
        {
          $set: {
            "role.numeric": Number(permission),
            "role.title": setup.userPermissions[permission],
          },
        },
        function (err) {
          if (err)
          ...
          if (!err)
            log.debug(
              adminUser.name +
                ...
                user.name +
                " to " +
                setup.userPermissions[permission]
            );
          cb();
        }
      );
    ...
    module.findOrCreateUser(null, alt.refreshToken, alt, function (AltUser) {
      ...
      if (AltUser.account != null && !AltUser.account.main) {
        status({
          type: "error",
          message:
            "This alt already belongs to someone, think this is an error? Contact leadership.",
        });
        return;
      }
      ...
      if (user.characterID == AltUser.characterID) {
        status({
          type: "error",
          message: "Error, you cannot link your main to itself.",
        });
        return;
      }
      ...
      if (AltUser.account.main && AltUser.account.linkedCharIDs.length > 0) {
        status({
          type: "error",
          message:
            "Error, you cannot link " +
            AltUser.name +
            " to your account as it's already a master account. To link these accounts, logout and into " +
            AltUser.name +
            " and then add this pilot as an alt.",
        });
        return;
      }
      ...
      db.updateOne(
        { characterID: AltUser.characterID },
        {
        ...
        function (err) {
          if (err)
            console.log("users.linkPilots - error updating alt account: ", err);
          if (!err) {
            db.updateOne(
              { characterID: account.mainID },
              { $push: { "account.linkedCharIDs": AltUser.characterID } },
              function (err) {
                if (err)
                  console.log(
                    "users.linkPilots - error updating main account: ",
                    err
                  );
              }
            );
      ...
      status({
        type: "success",
        message:
          alt.CharacterName + " has been added to your account as an alt.",
      });
    });
  };
  ...
  module.getMain = function (userID, mainPilot) {
    db.findOne({ characterID: userID }).then(function (userObject) {
      //No user detected
      if (!userObject) {
        console.log("No user detected " + userID, mainPilot);
        mainPilot(null);
        return;
      }
      ...
      db.findOne({ characterID: Number(userObject.account.mainID) }).then(
        function (userObject) {
          mainPilot(userObject);
        }
      );
  ...
  module.getAlts = function (userID, returnCharacters) {
    ...
    module.getMain(userID, function (mainObject) {
      ...
      knownPilots.push({
        characterID: mainObject.characterID,
  ...
  module.checkSkills = function (user, skillsPackage, cb) {
    userO.getRefreshToken(user.characterID, function (accessToken) {
      esi
        .characters(user.characterID, accessToken)
        .skills()
        .then((result) => {
          ...
          for (var i = 0; i < cSkillSet.length; i++) {
            cSkillSet[i].actual = esiSkills[cSkillSet[i].id]
              ? esiSkills[cSkillSet[i].id].current_skill_level
              : 0;
            //did skill fail?
            if (
              cSkillSet[i].actual < cSkillSet[i].required &&
              cSkillSet[i].failable == true
            ) {
          ...
          for (var c = 0; c < skillsPackage.categories.length; c++) {
            ...
            skillsPackage.categories[
              c
            ].txticon = `<i class="fa fa-check-circle"></i>`;
            ...
            for (var i = 0; i < cSkillSet.length; i++) {
              ...
              if (
                ...
                skillsPackage.categories[
                  c
                ].txticon = `<i class="fa fa-times-circle"></i>`;
              } else {
                cSkillSet[i].class = "skills-pass";
              }
            }
